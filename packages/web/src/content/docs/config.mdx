...
Provider options can include `timeout`, `chunkTimeout`, `setCacheKey`, and `cacheTtl`:
...
- `cacheTtl` - TTL for Anthropic-style prompt cache breakpoints. Either `"5m"` (default) or `"1h"`. The 1-hour cache costs 2× base input tokens on writes (vs 1.25× for 5m) but extends cache lifetime, which helps when you have gaps longer than 5 minutes between requests. Can be overridden per-shell with the `ENABLE_PROMPT_CACHING_1H` environment variable, or forced back to 5m with `FORCE_PROMPT_CACHING_5M`.
